...
function credentialFields(gateway) {
  const keys = gateway?.credentialKeys?.length
    ? gateway.credentialKeys
    : ["keyId", "keySecret"];
...
export default function PaymentSettings() {
    ...
    setForm(
      gateway
        ? {
            ...
            credentials: credentialFields(gateway),
          }
        : blank
    );
    setOpen(true);
  ...
  function updateCredential(index, field, value) {
    setForm({
      ...form,
      credentials: form.credentials.map((item, i) => (i === index ? { ...item, [field]: value } : item)),
    });
  }

  const editingKnown = gateways.some((g) => g.provider === form.provider && g.known);
  const activeGateway = gateways.find((g) => g.provider === form.provider);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Enable checkout methods and configure gateways without exposing stored credentials."
        action={
      ...
      />
      {loading ? (
        <LoadingState label="Loading payment gateways…" />
      ) : error ? (
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="font-display font-semibold text-ink">
                    {gateway.label || titleCase(gateway.provider)}
                  </h3>
                  <p className="text-xs text-muted mt-1">
                    {gateway.description || gateway.settlementAccount || "No settlement account"}
                  </p>
                </div>
                <button type="button" onClick={() => toggle(gateway)}>
                  <Badge tone={gateway.enabled ? "success" : "neutral"}>
                    {gateway.enabled ? "Enabled" : "Disabled"}
                  </Badge>
                </button>
              </div>
              {gateway.requiresCredentials !== false && (
                <div className="flex items-center gap-2 text-xs text-muted mb-2">
                  <KeyRound size={14} />
                  <span>
                    {gateway.credentialsSet || gateway.envConfigured
                      ? "Credentials configured"
                      : "Credentials not configured"}
                  </span>
                </div>
              )}
              {gateway.provider === "razorpay" && gateway.envConfigured && !gateway.credentialsSet && (
                <p className="text-[11px] text-muted mb-3">Using keys from server environment.</p>
              )}
              {gateway.provider !== "COD" && gateway.requiresCredentials === false && (
                <p className="text-[11px] text-muted mb-3">Uses Razorpay when online payments are enabled.</p>
              )}
              <Button variant="secondary" size="sm" onClick={() => configure(gateway)}>
                Configure
              </Button>
            </Card>
          ))}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Configure payment gateway">
        <form onSubmit={save}>
          <Field label="Provider">
            {editingKnown ? (
              <input disabled className={inputCls} value={form.provider} />
            ) : (
              <input
                required
                className={inputCls}
          ...
          {(activeGateway?.requiresCredentials !== false || !editingKnown) && (
            <>
              <div className="text-xs font-medium text-muted mb-2">
                Credentials (leave values blank to keep existing credentials)
              </div>
